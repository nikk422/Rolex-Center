...
const WishlistAndCartProvider = ({ children }) => {
  ...
  const wishlistCartReduce = (state, action) => {
    switch (action.type) {
      ...
      case "INCREASE":
        let incre = state.cart.map((items) =>
          items.id === action.payload.id
            ? { ...items, cartQ: items.cartQ + 1 }
            : items
        );
        return { ...state, cart: [...incre] };
      case "DECREASE":
        let decri = state.cart.map((items) =>
          items.id === action.payload.id
            ? { ...items, cartQ: items.cartQ - 1 }
            : items
        );
  ...
  const [wishlistCartState, dispatchWishlistCart] = useReducer(
    wishlistCartReduce,
    {
      wishlist: [],
      cart: [],
  ...
  return (
    <wishlistCartContext.Provider
      value={{ wishlistCartState, dispatchWishlistCart }}
    >
      {children}
    </wishlistCartContext.Provider>
  );
